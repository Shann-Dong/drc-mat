...
class CanvasImage:
    ...
    def __init__(self, canvas_frame, canvas):
        ...
        self.canvas.bind('<B1-Motion>', self.__left_mouse_motion) # scribble motion
        self.canvas.bind('<ButtonRelease-1>', self.__left_mouse_release) # scribble release
        ...
        self._scribble_callback = None
        self._scribble_release_callback = None

    def register_click_callback(self,  click_callback):
        self._click_callback = click_callback

    def register_scribble_callbacks(self, scribble_callback, release_callback):
        self._scribble_callback = scribble_callback
        self._scribble_release_callback = release_callback
    ...
    def __left_mouse_button(self, event):
        ...
        if coords is not None:
            ...
            if self._scribble_callback is not None:
                self._scribble_callback(x=coords[0], y=coords[1])

    def __left_mouse_motion(self, event):
        if self._scribble_callback is not None:
            coords = self._get_click_coordinates(event)
            if coords is not None:
                self._scribble_callback(x=coords[0], y=coords[1])

    def __left_mouse_release(self, event):
        if self._scribble_release_callback is not None:
            self._scribble_release_callback()
